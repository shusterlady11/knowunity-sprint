...
- `src/components/*` — built components, one folder each with its CSS and stories: `iconSlot`, `button`, `statusPill`, `answerCard`, `bottomCTA`, `topicPill`, `micButton`, `inputModeToggle`, `toggleGroup`, `loadingDots`, `recordingGlow`, `progressMeter`, `buttonIcon`. Values come from tokens only; check Storybook before building a new one.
