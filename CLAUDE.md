...
- `src/components/*` — built components, one folder each with its CSS and stories: `iconSlot`, `button`, `statusPill`, `answerCard`, `bottomCTA`, `topicPill`, `micButton`. Values come from tokens only; check Storybook before building a new one.
