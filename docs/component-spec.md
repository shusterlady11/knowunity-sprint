...
- Skeleton bars: `Space/300` (12px) tall, since 14px has no token; widths 100%, 88% and 60% of the card; one `font/lineHeight/body-md` (26px, the Body M line height) apart; fill `interactive/secondary`, as in Figma.
...
- S: Body S Bold title and Caption S Regular (11px) caption, left-aligned, `Space/050` between.

**Built** (`src/components/textBlock`, 2026-09-24). Same properties and defaults as Figma. The block hugs its text and both lines fill its width. Colors `text/primary` and `text/secondary`. Heights with the default text: 100, 60, 44 and 36px (Body M's line height is now 26px and Caption S is 11px on a 14px line). XL and L titles are Inter, not Greed Condensed (unlicensed trial font), so they're wider than in Figma (XL "Header" 265px against 247px). The text is a pair of paragraphs and doesn't pick a heading level.
...
**Built** (`src/components/expandableResultRow`, 2026-09-24). Props `state`, `tone` and `transcript` as in Figma, plus two required code props, `label` (the concept) and `question`, because Figma types both into the layers. The transcript is shown in curly quotes; with no transcript the row says "Not answered.", as the neutral variant does. The header is a real button (`aria-expanded`); `onToggle(nextState)` lets the parent keep one row open at a time. The row fills its container; padding `Space/400`, gap `Space/200`, corners `Radius/400`; detail indented `Space/700`. The whole top of the card is the tap target (58px with the 26px Body M line, over the 48px minimum). Chevron: right when collapsed, a quarter turn clockwise (pointing down) when open (Figma's -90 is counterclockwise-positive). Icons: Check, X, DotOutline in `feedback/success/onSubtle`, `feedback/error/onSubtle`, `interactive/onSecondary`; chevron `interactive/primary`. No motion on the chevron turn (no token for it yet). Same icon, label and chevron positions as Figma; a collapsed row is 58px with the new 26px Body M line (Figma's row layers may still have the old 24px line, which gives 56px).
