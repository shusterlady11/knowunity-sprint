...
An active-recall step for Knowunity, driven by voice. After a student reviews a section, they explain the key ideas out loud in their own words. They speak, Knowie answers in text.
...
After reviewing a section, a student has *seen* the material but hasn't proven they can *retrieve* it. The existing steps test recognition or application. None of them make a student pull a concept out of memory and say it.
...
Everything in the section above is open except placement, which is decided below. In particular:

- **Where it lives — decided.** Launched from a stepping-stone in the study plan: a light pass early in the plan, then a fuller pass at the end covering all key concepts from the section. Also reachable from the main chat, offered as an "explain out loud" option when a student names a topic they want to study.
...
1. **Decided.** When a student genuinely doesn't know a term, "Reveal answer" at the bottom of the screen opens an overlay containing the answer. The overlay can be dragged up to full screen and scrolled.
2. **Decided.** XP pays out by how the answer was earned: full value for an unaided pass, partial for a hinted pass, little or none for a revealed answer. A student who hints or reveals but then repeats the full answer unaided ("say it back") claws back a portion of that lost value. The streak counter increments only on unaided passes, not on every attempt or completion. Part of the session's XP is held back and confirms only once the results-screen breakdown renders, so the reward lands with the real picture rather than before it.
